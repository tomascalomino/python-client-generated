...
> ModelCopyAuthorization models_authorize_copy(model_copy_authorization_definition)
...
try:
    # Allows another speech resource (source) to copy a model to this speech resource (target).
    api_response = api_instance.models_authorize_copy(model_copy_authorization_definition)
    pprint(api_response)
...
# **models_copy**
> Operation models_copy(id, model_copy_authorization)
...
try:
    # Copies a model from one subscription to another.
    api_response = api_instance.models_copy(id, model_copy_authorization)
    pprint(api_response)
except ApiException as e:
